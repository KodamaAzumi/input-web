...
const pathParamsValidate = ajv.compile(pathParamsSchema);
// base64 にマッチする正規表現
...
module.exports.update = async (event) => {
  ...
  const { entityIds, entities } = JSON.parse(event.body);
  const { id, timestamp } = event.pathParameters;
  const bodyValid = bodyValidate(JSON.parse(event.body));
  const pathParamsValid = pathParamsValidate({ id, timestamp });
  let headers = {
  ...
  try {
    ...
    if (!bodyValid) {
      throw bodyValidate.errors;
    }

    if (!pathParamsValid) {
      throw pathParamsValidate.errors;
    }
    ...
    const version = unmarshalledItem.version + 1;
